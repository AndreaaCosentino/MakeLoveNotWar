...
for (index, colname) in enumerate(df):
    ...
    if type(it.iloc[0][colname]) == np.float64:
...
ax[0].fill_between(all_years, fra_exp, zero_points, where = np.array(fra_exp) >= np_zero, color = '#BFE7F1', label = "Spesa superiore alla media")
ax[0].fill_between(all_years, fra_exp, zero_points, where = np.array(fra_exp) < np_zero, color = '#DD8D81', label = "Spesa inferiore alla media")
ax[0].set_title("Francia", fontsize = 10)
ax[0].legend(loc = 'upper left')

ax[1].fill_between(all_years, uk_exp, zero_points, where = np.array(uk_exp) >= np_zero, color = '#BFE7F1')
ax[1].fill_between(all_years, uk_exp, zero_points, where = np.array(uk_exp) < np_zero, color = '#DD8D81')
ax[1].set_title("Regno Unito", fontsize = 10)

ax[2].fill_between(all_years, it_exp, zero_points, where = np.array(it_exp) >= np_zero, color = '#BFE7F1')
ax[2].fill_between(all_years, it_exp, zero_points, where = np.array(it_exp) < np_zero, color = '#DD8D81')
ax[2].set_title("Italia", fontsize = 10)

ax[3].fill_between(all_years, ger_exp, zero_points, where = np.array(ger_exp) >= np_zero, color = '#BFE7F1')
ax[3].fill_between(all_years, ger_exp, zero_points, where = np.array(ger_exp) < np_zero, color = '#DD8D81')
ax[3].set_title("Germania", fontsize = 10)
ax[3].set_xticks(["1960", "1980","2000", "2020"])
