...
ax[0].fill_between(all_years, usa_diff, zero_points, where = np.array(usa_diff) < np_zero, color = '#DD8D81', label = "Spent inferiore alla media")
ax[0].set_title("Stati Uniti d'America", fontsize = 10)

ax[0].legend(loc = 'upper right')

ax[1].fill_between(all_years, fra_diff, zero_points, where = np.array(fra_diff) >= np_zero, color = '#BFE7F1')
ax[1].fill_between(all_years, fra_diff, zero_points, where = np.array(fra_diff) < np_zero, color = '#DD8D81')
ax[1].set_title("Francia", fontsize = 10)

ax[2].fill_between(all_years, uk_diff, zero_points, where = np.array(uk_diff) >= np_zero, color = '#BFE7F1')
ax[2].fill_between(all_years, uk_diff, zero_points, where = np.array(uk_diff) < np_zero, color = '#DD8D81')
ax[2].set_title("Regno Unito", fontsize = 10)

ax[3].fill_between(all_years, it_diff, zero_points, where = np.array(it_diff) >= np_zero, color = '#BFE7F1')
ax[3].fill_between(all_years, it_diff, zero_points, where = np.array(it_diff) < np_zero, color = '#DD8D81')
ax[3].set_title("Italia", fontsize = 10)

ax[4].fill_between(all_years, ger_diff, zero_points, where = np.array(ger_diff) >= np_zero, color = '#BFE7F1')
ax[4].fill_between(all_years, ger_diff, zero_points, where = np.array(ger_diff) < np_zero, color = '#DD8D81')
